  ...
  <div>
    <el-form ref="form" label-width="80px" :model="spu">
      ...
      <el-form-item label="SPU图片">
        <!--上传图片: action图片上传的地址 list-type: 文件列表的类型 on-preview:图片预览的时候会出发on-remove:当删除图片的时候会出发-->
        <el-upload
        ...
        <el-table style="width: 100%" :data="spu.spuSaleAttrList">
          ...
          <el-table-column prop="prop" label="属性值名称列表" width="width">
            <template slot-scope="{ row }">
              <el-tag :key="tag.id" v-for="(tag,index) in row.spuSaleAttrValueList" @close="row.spuSaleAttrValueList.splice(index,1)" closable :disable-transitions="false" > {{tag.saleAttrValueName}}</el-tag >
...
export default {
  ...
  data() {
    return {
      ...
      saleAttrList: [], //全平台的销售属性
      attrIdAndAttrName:'', //收集未选择销售属性的id name
    };
  },
  methods: {
    // 照片墙删除某一个图片的时候回触发
    ...
    addSaleAttrValue(row){
      ...
      // 给对象添加属性，不能直接添加，需要通过$set，因为vue底层监测不到
    ...
    handleInputConfirm(row){
      ...
      //修改inputvis ible为false，不就显示button
