<template>
  <div>
    <el-card style="margin: 20px 0px">
      <CategorySelect @categoryInfo="categoryInfo" :show="!isShow"></CategorySelect>
    </el-card>
    <el-card>
      <!-- 添加属性|修改属性的结构 -->
      <div v-show="isShow">
      ...
      <div v-show="!isShow">
        ...
        <el-table  border style="width: 100%; margin: 20px 0px" :data="attrInfo.attrValueList">
          ...
          <el-table-column prop="prop" label="操作" width="width">
            <template slot-scope="{row,$index}">
              <el-popconfirm :title="`你确定删除${row.valueName}？`" @onConfirm="deleteAttr($index)">
                <el-button type="danger" icon="el-icon-delete" size="mini" slot="reference"></el-button>
              </el-popconfirm>
            </template>
          </el-table-column>
        </el-table>
        <el-button type="primary" @click="addOrUpdeteAttr" :disabled="attrInfo.attrValueList.length<1">保存</el-button>
        <el-button @click="isShow = true">取消</el-button>
      </div>
    </el-card>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      attrInfo: {
        ...
        // 注意1:别再data当中收集三级分类的id因为对象存储数据无序存储|
        // 不能这样写： categoryId:this.category3Id
  ...
  methods: {
    ...
    async getAttrList() {
      ...
      let result = await this.$API.attr.reqCategoryInfoList(
        category1Id,
        category2Id,
        category3Id
      );
    ...
    addValue(){
      //向属性值的数组里面添加元素
      //attrId:是你相应的属性的id，目前而言我们是添加属性的操作，还没有相应的属性的id，目前而言带绘服务器的id为undefined
      //valueName:相应的属性值的名称
    ...
    toLook(row){
      ...
      if(row.valueName.trim() == ''){
        ...
        return; //执行了return 后面的代码就不会在执行了
      }
      // 新增的属性值不能与已有的属性值重复
      let isRepat = this.attrInfo.attrValueList.some((item)=>{
        //需要将row从效组里面判断的时候去除
        //row最新新增的属性值【数组的最后一项元素】
        //判断的时候,需要把己有的数组当中新增的这个属性值去除
        if(row !== item){
    ...
    async addOrUpdeteAttr(){
      ...
      try{
        // 发送请求
         await this.$API.attr.reqAddOrUpdeteAttr(this.attrInfo)
        //  展示品台属性的信号量进行切换
        this.isShow = true;
        // 提示消息
