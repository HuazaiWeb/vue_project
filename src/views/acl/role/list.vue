  ...
  <div>
    ...
    <el-table
      ...
      <el-table-column label="操作" width="300" align="center">
        <template slot-scope="{row}">
          <el-button type="info" size="mini" icon="el-icon-info" title="分配权限" @click="$router.push(`/acl/role/auth/${row.id}?roleName=${row.roleName}`)"></el-button>
          <!-- <HintButton size="mini" type="info" icon="el-icon-info" title="分配权限"
            @click="$router.push(`/acl/role/auth/${row.id}?roleName=${row.roleName}`)"/> -->
          
          <el-button type="primary" size="mini" icon="el-icon-check" title="确定" @click="updateRole(row)" v-if="row.edit"></el-button>
          <!-- <HintButton size="mini" type="primary" icon="el-icon-check" title="确定" 
            @click="updateRole(row)" v-if="row.edit"/> -->

          <el-button type="primary" size="mini" icon="el-icon-edit" title="修改角色" @click="row.edit= true" v-if="!row.edit"></el-button>
            
          <!-- <HintButton size="mini" type="primary" icon="el-icon-edit" title="修改角色" 
            @click="row.edit= true" v-if="!row.edit"/> -->

          <el-button type="danger" size="mini" icon="el-icon-delete" title="删除角色" @click="removeRole(row)"></el-button>
